...
int menu1() {   //셔틀(상행)버스 메뉴 선택 출력 함수
	...
	printf("  2. 입력된 시간에서 가장 가까운 지하철 시간 보기\n\n");
	printf("  3. 버스 전체 시간표 출력하기 \n\n");
	printf("  4. 버스 위치 검색하기\n\n");
	printf("  5. 종료 \n\n\n");
	printf("==================================================\n\n");
	printf("       원하는 서비스의 번호를 입력해 주세요\n\n ");
...
int menu2() {   //셔틀(하행)버스 메뉴 선택 출력 함수
	printf("\n================ < 하 행 버 스 > =================");
	printf("\n\n                 (학교 -> 신창역)\n\n");
	printf("\n\n  1. 입력된 시간에서 가장 가까운 버스 시간 보기\n\n");
...
void search_bus(int list[], int n, int key)  //가장 가까운 버스가 몇 번째 배열인지 순차탐색하는 함수
{
	...
	while (list[i] != key)
	{
		for (i = 0; i < n; i++) { //n(SIZE)까지 배열을 비교
			if (key == list[i]) { //키랑 list[i]가 같으면 출력
				printf("\n\n        가장 가까운 버스는 %d시 %d분입니다\n\n", list[i] / 60, list[i] % 60);
		...
		if (key > 14400) { //키가 24*60을 넘어가면 시간 오류 출력
			printf("\n\n           잘못된 시간이 입력되었습니다\n\n");
			printf("              선택창을 재실행합니다\n\n");
			Sleep(1500);
			system("cls");
...
void search_subway(int list[], int n, int key)  //가장 가까운 지하철이 몇 번째 배열인지 순차탐색하는 함수
{
	int i = 0;
	int o_key = key; // 원본 키를 o_key로 복사한다
	while (list[i] != key)
...
int make_new_profile(struct user* u) {   //새 사용자 정보 저장 함수
	FILE* fp;
	char filename[MAX] = "\0";
	strcpy(filename, u->directory);
	strcat(filename, "/");
...
int load_profile(struct user* u) {  //사용자 정보 확인 함수
	FILE* fp;
	char* p;
	int count = 0;
	char str_tmp[MAX];
...
int money_charger(struct user* u) {   //금액 충전 함수
	int input = 0;
	system("cls");
	while (1) {
		printf("\n=================================================\n");
...
int user_search(struct user* u) {   //로그인 이력 확인 함수
	char user[] = { "./user" };
	char id[30] = { "./user/" };
	char temp[9] = { "\0" };
	int id_forder = 0;
...
int reservation_bus(struct user* u) {   //버스 예약 함수
	FILE* fp = NULL;
	FILE* fp2 = NULL;
	char filename[MAX] = "\0";
...
int check_bus(struct user* u) {   //예매 내역 확인 함수
	FILE* fp;
	char* p;
	int count = 0;
	int total = 0;
...
int print_menu() {   //통학버스 메뉴 화면 출력 함수
	int input = 0;
	printf("\n=================================================\n");
	printf("                   인천 -> 학교\n");
	printf("=================================================\n\n");
...
int login(struct user* u) {   //로그인 함수
	struct user* temp = u;
	char name[MAX];
	int id = 0;
	printf(" ================================================\n\n");
...
void BusPosition() {   //통학버스 현재 위치 확인 함수
	int total = 0;
	int now = 0;
	int now_case = 0;
	time_t t = time(NULL);
	...
	if (total >= 390 && total <= 450) {
		...
		case 0:
			...
			break;
		case 1:
			printf("\n  문학IC에서 '주안, 문학경기장' 방면으로 주행 중\n\n");
			break;
		case 2:
			printf("\n 월곶분기점에서 '인천' 방면으로 왼쪽 도로 주행 중\n\n");

			break;
		case 3:
			printf("\n서평택분기점에서 '서울, 발안' 방면으로 오른쪽 방향\n\n");
			break;
		case 4:
			printf("\n    청북IC에서 '청북IC' 방면으로 오른쪽 고속도로\n");
			printf("                    진입 중\n\n");

			break;
		case 5:
			printf("\n 읍내교차로에서 '아산, 아산방조제, 공주' 방면으로\n");
			printf("                  오른쪽 방향\n\n");
			break;
		case 6:
			printf("\n                순천향대학교 후문\n\n");
	...
	while (1) {  //무한루프 실행 (메인 화면이 종료 번호 입력 외에는 종료되지 않도록 하기 위함)
		...
		if (ser.bus_type == 1)  //셔틀버스 번호 입력 시
		{
			while (1)   //무한루프 실행 (셔틀버스 화면이 종료 번호 입력 외에는 종료되지 않도록 하기 위함)
			{
				...
				case 1:   //상행 번호 입력 시
					system("cls");
					FILE* fp = NULL;   //버스 시간 저장
					int list[10000] = { 0 };
					FILE* fp_ = NULL;   //지하철 시간 저장
					int list_[10000] = { 0 };
					int c;
					while (1)   //무한루프 실행 (상행 메뉴 화면이 종료 번호 입력 외에는 종료되지 않도록 하기 위함)
					{
						...
						fp_ = fopen("subway.txt", "r");
						int save_size2;
						for (save_size2 = 0; !feof(fp_); save_size2++) // 파일이 끝날 때까지 list_에 저장
						{
							fscanf(fp_, "%d", &list_[save_size2]);
						}
						int SIZE2 = save_size2 - 1; 
						fclose(fp_);
						...
						case 1:   //가장 가까운 버스 시간 찾기 번호 입력 시
							...
							break;
						case 2:   //가장 가까운 지하철 시간 찾기 번호 입력 시
							RemoveScrollbar();
							system("cls");
							printf("\n===============================================");
							printf("\n\n       가장 가까운 지하철의 시간을 출력합니다\n");
							int h2, m2, key_;
							key_ = get_trans_time();  //시간과 분을 입력받고 시간에 *60을 하고 분을 더해 반환하는 함수
							search_subway(list_, SIZE2, key_);  //가장 가까운 지하철 몇 번째 배열인지 순차 탐색하는 함수
							break;
						case 3:   //버스 전체 시간표 번호 입력 시
							RemoveScrollbar();
							printf("\n\n");
			...
			case 1:   //로그인 실행 번호 입력 시
				...
				if (login(&u) == 1) {
					while (1) //무한루프 실행
					{
						...
						switch (menu_input)
						{
						case 1:  //금액 충전 번호 입력 시
							money_charger(&u);

							break;
						case 2:   //예매 내역 번호 입력 시
							system("cls");
