...
int reservation_bus(struct user* u) {
	...
	printf("-------------------------------\n");
	if (u->cash >= 6000) {
		printf("예매하려는 버스의 날짜를 입력해주세요. (ex>20211126) : ");
		scanf("%d", &reservation_day);
		fflush(stdin);
		printf("버스 종류를 입력해주세요. (등교 or 하교) : ");
		scanf("%s", bus_cate);
		strcat(filename2, "/");
...
int print_menu() {
	...
	printf("1. 금액 충전\n");
	printf("2. 예매 내역 확인\n");
	printf("3. 통학 버스 예매 하기\n");
	printf("4. 통학 버스 위치 확인\n");
	printf("-------------------------------\n");
	printf("해당 번호를 입력해주세요 : ");
	scanf("%d", &input);
	return input;
}
